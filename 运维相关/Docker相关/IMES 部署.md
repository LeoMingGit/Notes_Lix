...
```text
tar -cvzf IMES.tar "E:\代码练习\IMes-Pro\iMES.Net\iMES.WebApi\bin\Release\IMES"
```
...
```
docker run -d -p  9991:9991 imesv1
```
...
```
docker run -it -p 9991:9991  imesv1
```



在linux上发起请求，验证服务是否跑起来了

```
curl http://localhost:3738

curl http://localhost:42663

curl http://localhost:9991 

curl  http://localhost:9991/index.html
```
...
docker exec -it imesv1 /bin/bash 
```
